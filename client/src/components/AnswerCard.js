  ...
  return (
    <div className="answer__card">
      {!editState && (
        <React.Fragment>
          {showQuestion && (
            <div className="title">{answer.question.description}</div>
          )}
            ...
            {answer.author._id === userId && (
              <div className="edit">
                ...
                <DeleteOutlineOutlined
                  onClick={() => {
                    Delete(answer._id, answer.author._id);
                  }}
                />
              </div>
            )}
          </div>
          <div className="answer__des">{answer.description}</div>
        </React.Fragment>
      )}

      {editState && (
        <UpdateAnswer answer={answer} reload={reloadAns} cancel={cancelEdit} />
      )}
    </div>
  );
};
